...
class PageParser(BaseParser):
    ...
    async def parse_table(page: Page) -> list[ScrapeResonse]:
        ...
        iframe = await page.wait_for_selector("iframe", timeout=30000)
        ...
        # 步骤1：确保表格可见
        # 等待表格刷新（DOM 更新）
        await frame.wait_for_selector("table.cq_resultTable:has(tbody tr)", state="attached", timeout=30000)
    
        # 步骤2：等待行数据加载
        # await PageParser.wait_for_table_rows(page)

        row_data = await frame.evaluate('''() => {
            const rows = document.querySelectorAll('table.cq_resultTable tbody tr');
            return Array.from(rows).slice(1).map(row => {
                ...
                return columns.length > 0 ? {
                    ...
                    admission_type: columns[5].innerText.trim(),
